...
SESSION_MAP_PATH=/フルパス/Claude-Code-Remote-Tel/src/data/session-map.json
```
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "if [ -n \"$TMUX\" ]; then node /フルパス/Claude-Code-Remote-Tel/claude-hook-notify.js completed; fi",
            "timeout": 5
          }
        ]
      }
