...
class TelegramWebhookHandler {
    ...
    async _handleMessage(message) {
        ...
        // Parse command - support /cmd TOKEN command format
        ...
        // Support TOKEN command format (without /cmd)
        ...
        // Direct message mode - send directly to tmux session without token
        try {
            await this.injector.injectCommand(messageText, 'claude-tel');
            this.logger.info(`Direct command injected - User: ${chatId}, Command: ${messageText}`);
        } catch (error) {
            this.logger.error('Direct command injection failed:', error.message);
