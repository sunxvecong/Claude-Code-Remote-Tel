...
class TelegramChannel extends NotificationChannel {
    ...
    _generateTelegramMessage(notification, sessionId, token) {
        ...
        let messageText = `${emoji} *${notification.project}*\n\n`;

        if (notification.metadata?.claudeResponse) {
            messageText += notification.metadata.claudeResponse.substring(0, 500);
            if (notification.metadata.claudeResponse.length > 500) {
                messageText += '...';
            }
        } else {
            messageText += notification.message || 'Task ' + type;
        }
