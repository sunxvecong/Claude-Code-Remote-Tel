    ...
    extractConversation(text, sessionName = null) {
        ...
        claudeResponse = claudeResponse
            ...
            .filter(line => {
                ...
                if (/^\s*esc to interrupt\s*$/.test(trimmed)) return false;
                if (/^\*\s*Seasoning/.test(trimmed)) return false;
                if (/running stop hooks/i.test(trimmed)) return false;
                if (/^[✽✻✺✹✸✷✶✵✴✳✲✱✰⏺◐◑◒◓⣾⣽⣻⢿⡿⣟⣯⣷⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏]/.test(trimmed)) return false;
                if (/^\s*◐|◑|◒|◓/.test(trimmed)) return false;
                if (/medium\s*·\s*\/effort/.test(trimmed)) return false;
                return true;
            })
            .join('\n')
            .replace(/[╭╰│╮╯]/g, '')
